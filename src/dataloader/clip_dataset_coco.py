...
class ClipDatasetCOCO(BaseDataset):
    """
    Clip Dataset.
    ...
    def __init__(self, root_dir, meta_file, transform=None,
        ...
        super(ClipDatasetCOCO, self).__init__(root_dir=root_dir,
                                              meta_file=meta_file,
                                              read_from=read_from,
                                              transform=transform,
                                              evaluator=evaluator)
        ...
        if self.rank_dataset:
            if self.fseek:
                ...
                with open(self.meta_file[source_id]) as f:
                    ...
                    # filename = joinpath(self.root_dir[source_id], meta['images']['file_name'])
    ...
    def _get_label_text(self, text):
        ...
        if self.label_texts_ensemble == 'prompt6':
            f = f'{osp.abspath(os.getcwd())}./prompts/query_pattern_prompt6'
        elif self.label_texts_ensemble == 'prompt8':
            f = f'{osp.abspath(os.getcwd())}./query_pattern_prompt8'
        elif self.label_texts_ensemble == 'prompt80':
            f = f'{osp.abspath(os.getcwd())}./prompts/query_pattern_prompt80'
            ...
            f = f'{osp.abspath(os.getcwd())}./prompts/query_pattern_prompt1'
        else:
...
# current class is executed in dataloader
class ClipDatasetRankedCOCO(ClipDatasetCOCO):
    def __init__(self, root_dir, meta_file, transform=None,
                 read_from='mc', evaluator=None, image_reader_type='pil',
        ...
        super(ClipDatasetRankedCOCO, self).__init__(root_dir, meta_file, transform,
                                                    read_from, evaluator, image_reader_type, server_cfg, fseek,
                                                    label_texts_ensemble, rank_dataset)
        ...
        if self.fseek:
            ...
            for each_meta_file in meta_files:
                ...
                if self.offset_file_prefix:
                    offset_file = os.path.join(self.offset_file_prefix,
                                               each_meta_file.replace('/', '_').replace('\\', '_'))
                    offset_ranked_dic = os.path.join(self.offset_file_prefix, 'ranked_offsets')
                    offset_ranked_file_dic = os.path.join(offset_ranked_dic,
                                                          each_meta_file.replace('/', '_').replace('\\', '_').replace(
                                                              '.', '_'))
                        ...
                        with open(offset_file, 'rb') as f:
                            while True:
                                ...
                                if number % 1000000 == 0 and self.rank == 0:
                                    print(
                                        '-------------------------------------------------------------- every 1M time:',
                                        (time.time() - star_time), '{}M'.format(number / 1000000))
                    else:
                        link.barrier()
                        if self.rank != 0:
                            offset_file = None
                ...
                if self.rank % 8 == 0:
                    print('[Rank %d] Number of file' % self.rank, each_meta_file, len(ranked_line_offset[self.rank]),
                          flush=True)
            ...
            if self.rank == 0:
                print(f'[Rank {self.rank}] Dataset Offset Loading Done! Ranked_num range is {min_num, max_num}',
                      flush=True)
            if max_num > self.num:
                diff = max_num - self.num
                self.ranked_line_offsets[self.rank][0].extend(
                    random.sample(self.ranked_line_offsets[self.rank][0], diff))
                self.num += diff
            else:
                ...
                with open(each_meta_file) as f:
                    ...
                    for line in f:
                        ...
                        if seed_num % 1000000 == 0 and self.rank == 0:
                            print('-------------------------------------------------------------- every 1M time:',
                                  (time.time() - star_time), '{}M'.format(seed_num / 1000000))
                    # print('-------------------------',rd,ranked_num)
