...
class PatchDropout(nn.Module):
    ...
    def update_patch_indices(self, patch_indices_keep):
    ...
    def forward(self, x):
        ...
        if self.patch_indices_keep is not None:
            patch_indices_keep = self.patch_indices_keep
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # image_height, image_width = self.image_size = to_2tuple(224)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if(x.shape[1] > self.positional_embedding.shape[0]):
            ...
            x = x + self.interpolate_pos_encoding(x, dim, dim).to(x.device)
        else:
