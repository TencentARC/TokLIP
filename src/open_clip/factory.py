...
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, DistillSimpleClipLoss
from .pretrained import is_pretrained_cfg, get_pretrained_cfg, download_pretrained,\
    list_pretrained_tags_by_model, download_pretrained_from_hf
from .transform import image_transform_v2, AugmentationCfg, PreprocessCfg, merge_preprocess_dict, merge_preprocess_kwargs, image_transform_v2_ori
from .tokenizer import HFTokenizer, SimpleTokenizer, SigLipTokenizer, DEFAULT_CONTEXT_LENGTH
from .model_distill import CLIP as CLIP_DISTILL
from .model_distill import CustomTextCLIP as CustomTextCLIP_DISTILL
...
def create_model(
        ...
        init_visual_weights: bool = False,      # NOTE whether to load visual weights
        distill: bool = False,                  # NOTE whether to load distillation model
        init_ori_logit_scale: bool = False,         # NOTE whether to initialize logit_scale
        high_resolution: bool = False,           # NOTE whether to load high resolution model
        **model_kwargs,
):
    ...
    if custom_text:
        ...
        if "multimodal_cfg" in model_cfg:
            ...
            model = CustomTextCLIP_DISTILL(**model_cfg, cast_dtype=cast_dtype)
        else:
            model = CustomTextCLIP(**model_cfg, cast_dtype=cast_dtype)
    else:
        # NOTE load distillation model
        if distill:
            model = CLIP_DISTILL(**model_cfg, cast_dtype=cast_dtype, distill=distill)
        else:
    ...
    if pretrained:
        ...
        if pretrained_cfg:
            ...
            print("================load from checkpoint_path: ", checkpoint_path)


        if checkpoint_path:
            # logging.info(f'Loading pretrained {model_name} weights ({pretrained}).')
            # load_checkpoint(model, checkpoint_path, weights_only=load_weights_only)
            logging.info(f'Loading vision and text pretrained {model_name} weights ({pretrained}).')
            
            checkpoint = torch.load(checkpoint_path, map_location='cpu')
            ckpt_state_dict = checkpoint['state_dict'] if 'state_dict' in checkpoint else checkpoint

            # remove 'module.' prefix
            if list(ckpt_state_dict.keys())[0].startswith('module.'):
                print("Removing 'module.' prefix from checkpoint keys.")
                ckpt_state_dict = {k[7:]: v for k, v in ckpt_state_dict.items()}  # 去除 'module.' 前缀

            if init_visual_weights:
                # remove visual part in ckpt_state_dict
                ckpt_state_dict = {k: v for k, v in ckpt_state_dict.items() if not k.startswith('visual.')}
            
            if init_ori_logit_scale:
                # deelte logit_scale
                ckpt_state_dict = {k: v for k, v in ckpt_state_dict.items() if not k.startswith('logit_scale')}
            ...
            if high_resolution:
                # breakpoint()
                positional_embedding = ckpt_state_dict['visual.positional_embedding']
                # use this positional embedding to replace the model's positional embedding
                visual_pos_embed = torch.nn.Parameter(positional_embedding)
                model.visual.positional_embedding = visual_pos_embed
                print("**************Positional embedding replaced with pretrained weights.")

            # select the keys in ckpt_state_dict that are in model_state_dict and have the same shape
            filtered_state_dict = {k: v for k, v in ckpt_state_dict.items() if k in model_state_dict and model_state_dict[k].shape == v.shape}

            # load the filtered state_dict
            model.load_state_dict(filtered_state_dict, strict=False) 

            # print the keys that are loaded and skipped
            loaded_keys = filtered_state_dict.keys()
            skipped_keys = [k for k in ckpt_state_dict.keys() if k not in loaded_keys]
            print(f"Loaded keys: {list(loaded_keys)}")
    ...
    if getattr(model.visual, 'image_size', None) is not None:
        print("model.visual.image_size: ", model.visual.image_size)
...
def create_loss(args):
    if args.distill:
        if args.siglip_train:
            return DistillSimpleClipLoss(
...
def create_model_and_transforms(
        ...
        distill: bool = False,
        init_ori_logit_scale: bool = False,
    ...
    model = create_model(
        ...
        init_visual_weights=init_visual_weights,
        distill=distill,
        init_ori_logit_scale=init_ori_logit_scale,
    ...
    if distill:
        preprocess_train = image_transform_v2_ori(
            pp_cfg,
            is_train=True,
            aug_cfg=aug_cfg,
        )  
